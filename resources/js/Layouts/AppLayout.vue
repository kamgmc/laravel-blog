    ...
    <nav class="bg-white border-b border-gray-100">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
          <div class="flex">
            ...
            <div v-if="user" class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
          ...
          <div v-if="!user" class="flex">
            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
              <a
                ...
                :href="route('login')"
                @click.prevent="redirect(route('login'))">
                Login
              </a>
            </div>
            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
              <a
                class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out"
                :href="route('register')"
                @click.prevent="redirect(route('register'))">
                Register
              </a>
            </div>
          </div>
              ...
              <jet-dropdown align="right" width="48">
                ...
                <template #content>
                  ...
                  <template v-if="$page.jetstream.hasTeamFeatures">
                    ...
                    <!-- Team Settings -->
                    <jet-dropdown-link :href="route('teams.show', user.current_team)">
                      Team Settings
                    </jet-dropdown-link>
                      ...
                      <form @submit.prevent="switchToTeam(team)" :key="team.id">
                        <jet-dropdown-link as="button">
                          <div class="flex items-center">
                            <svg v-if="team.id === user.current_team_id" class="mr-2 h-5 w-5 text-green-400"
      ...
      <div :class="{'block': showingNavigationDropdown, 'hidden': ! showingNavigationDropdown}" class="sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
          ...
          <jet-responsive-nav-link v-if="user" :href="route('admin')" :active="route().current('admin')">
            Posts
          </jet-responsive-nav-link>
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
          ...
          <div v-else class="mt-3 space-y-1">
            ...
            <template v-if="$page.jetstream.hasTeamFeatures">
              ...
              <!-- Team Settings -->
              <jet-responsive-nav-link :href="route('teams.show', user.current_team)"
              ...
              <template v-for="team in user.all_teams">
                <form @submit.prevent="switchToTeam(team)" :key="team.id">
                  <jet-responsive-nav-link as="button">
                    <div class="flex items-center">
                      <svg v-if="team.id === user.current_team_id" class="mr-2 h-5 w-5 text-green-400" fill="none"
...
export default {
  ...
  methods: {
    redirect(route) {
      window.location = `/${route.urlBuilder.route.uri}`
    },
