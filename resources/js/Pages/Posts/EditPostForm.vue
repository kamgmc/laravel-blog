          ...
          <div class="md:grid md:grid-cols-3 md:gap-6">
            <div class="md:col-span-1">
              <div class="px-4 sm:px-0">
                <h3 class="text-lg font-medium leading-6 text-gray-900">Update "{{post.title}}"</h3>
                <p class="mt-1 text-sm text-gray-600">
                  You can update your posts any time you want.
                </p>
              </div>
            </div>
    ...
    updatePost() {
      if (this.validateForm()) {
        axios.put(`/api/posts/${this.post.id}`, this.post)
          ...
          .catch(reason => console.warn('Error updating post', reason.response))
      }
    },
    validateForm() {
      this.errors = [];
